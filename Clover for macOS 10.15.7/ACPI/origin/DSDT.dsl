 ...
 *
 * Disassembly of DSDT.aml, Thu Dec 17 23:43:36 2020
 *
...
DefinitionBlock ("", "DSDT", 2, "SECCSD", "LH43STAR", 0x01072009)
{
    ...
    External (_PR_.NPSS, UnknownObj)    // (from opcode)
    ...
    External (_PR_.PSTE, UnknownObj)    // (from opcode)
    ...
    External (_SB_.NTCA, MethodObj)    // 1 Arguments (from opcode)
    ...
    External (_SB_.PCI0.GFX0.LBTU, MethodObj)    // 1 Arguments (from opcode)
    External (_SB_.PCI0.GFX0.PDRD, MethodObj)    // 0 Arguments (from opcode)
    ...
    External (_SB_.PCI0.LPCB.H_EC.BAT1, UnknownObj)    // Warning: Unknown object
    ...
    External (_SB_.SCAI, DeviceObj)    // (from opcode)
    ...
    External (NVGF, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    External (NVTP, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    ...
    External (VGTY, UnknownObj)    // (from opcode)    // Conflicts with a later declaration
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xC4A75000, 0x077B)
    ...
    OperationRegion (SSDB, SystemMemory, 0xC4A63838, 0x0040)
                ...
                If (LEqual (Arg0, ToUUID ("a69f886e-6ceb-4594-a41f-7b5dce24c553")))
                {
                    Switch (ToInteger (Arg2))
                    {
                        ...
                        Case (0x03)
                        {
                            ...
                            If (LEqual (Arg3, ToUUID ("b55eba24-8349-4dad-afc7-8489d23c93ef")))
                            {
                                Return (One)
                            }

                            If (LEqual (Arg3, ToUUID ("bb303551-0914-4d56-a3ff-20955c598a8f")))
                            {
                                Return (One)
                            }
    ...
    Mutex (MTX1, 0x00)
    OperationRegion (SAWB, SystemMemory, 0xC4A7AA29, 0x020A)
    ...
    Scope (_SB.PCI0)
    {
        Device (AMW0)
        {
            ...
            Name (_WDG, Buffer (0x3C)
            {
                ...
                /* 0028 */  0xBA, 0x47, 0x6C, 0xC1, 0xE3, 0x50, 0x4A, 0x44,
                /* 0030 */  0xAF, 0x3A, 0xB1, 0xC3, 0x48, 0x38, 0x00, 0x02,
                /* 0038 */  0x30, 0x31, 0x01, 0x02                         
            })
    ...
    Scope (_SB.PCI0.I2C0)
    {
        Device (SPTP)
        {
            ...
            Method (_INI, 0, NotSerialized)  // _INI: Initialize
            {
                ...
                ElseIf (LEqual (TPTY, One))
                {
                    Store (0x15, Local0)
                }
                ElseIf (LEqual (TPTY, 0x03))
                {
                    Store (0x4A, Local0)
                }
                ElseIf (LEqual (TPTY, 0x04))
                {
                    Store (0x4A, Local0)
                }
                Else
    ...
    OperationRegion (SNVS, SystemMemory, 0xC4A7AD18, 0x00B1)
    ...
    Scope (_SB)
    {
        Method (WAK1, 1, NotSerialized)
        {
            If (LEqual (ECON, One))
            {
                Store (^PCI0.LPCB.H_EC.ACEX, PWRS)
                GSSM (0x9A, PWRS)
            }
        }
    ...
    Scope (_SB)
    {
        ...
        Method (PPTS, 1, NotSerialized)
        {
            ...
            If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
            {
                Store (^PCI0.LPCB.H_EC.B1AF, Local0)
            ...
            Store (Zero, NVGF)
            Store (^PCI0.LPCB.H_EC.DCBE, SDCK)
        ...
        Method (PWAK, 1, NotSerialized)
        {
            ...
            If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
            {
                If (LNotEqual (SDCK, ^PCI0.LPCB.H_EC.DCBE))
                {
                    Store (^PCI0.LPCB.H_EC.DCBE, SDCK)
                }
                ...
                Store (^PCI0.LPCB.H_EC.DPTE, ^PCI0.LPCB.H_EC.DPPF)
            }

            If (LEqual (ECON, One))
            {
                If (LNotEqual (^PCI0.LPCB.H_EC.ACEX, PWRS))
                {
                    Store (^PCI0.LPCB.H_EC.ACEX, PWRS)
                    CCRN ()
                }

                If (LNotEqual (^PCI0.LPCB.H_EC.TIST, Zero))
                {
                    If (LAnd (LEqual (And (CPTY, 0x80), 0x80), LEqual (^PCI0.LPCB.H_EC.TIST, One)))
                    {
                        GSSM (0xAA, Zero)
                    }

                    Store (^PCI0.LPCB.H_EC.TZCH (), RIST)
            ...
            Store (GPC0 (0x02010004), Local0)
            Store (And (Local0, 0xFFFFFEFE), Local1)
    ...
    Scope (_SB)
    {
        ...
        Device (PWRB)
        {
            ...
            Method (_PRW, 0, NotSerialized)  // _PRW: Power Resources for Wake
            {
                ...
                Store (GGPE (0x02010010), Local0)
                Store (Local0, Index (PRWS, Zero))
                Return (PRWS)
            }
        }
    }
    ...
    Scope (_SB.PCI0.LPCB.H_EC)
    {
        ...
        {
            P8XH (Zero, 0x73)
            If (LAnd (DPTE, LAnd (LEqual (SLFL, Zero), LEqual (PPCS, Zero))))
            {
                Return (Zero)
            }

            If (LAnd (DPTE, LAnd (LNotEqual (SLFL, Zero), LNotEqual (PPCS, Zero))))
            {
                Return (Zero)
            }

            If (LAnd (LEqual (And (CPTY, 0x80), 0x80), LEqual (TIST, One)))
            {
                GSSM (0xAA, Zero)
            }

            If (LAnd (LEqual (And (CPTY, 0x80), 0x80), LEqual (TIST, Zero)))
            {
                GSSM (0xAA, One)
            }
    ...
    Scope (_SB)
    {
        Device (SCAI)
        {
            ...
            Method (CSXI, 1, Serialized)
            {
                ...
                Store (0xFF, RFLG)
                If (LAnd (LEqual (SAFN, 0x5843), LEqual (SASB, 0x90)))
                {
                    ...
                    If (LEqual (GUNM, 0x80))
                    {
                        ...
                        Store (Zero, GUNM)
                        If (LAnd (LEqual (VGTY, 0x02), LEqual (M110, One)))
                        {
                            Increment (GUNM)
                            Store (ToUUID ("8246028d-8bca-4a55-ba0f-6f1e6b921b8f"), GUD1)
                        }

                        Increment (GUNM)
                        Store (ToUUID ("415784c7-8236-45ac-ba08-8d5dfa86d094"), GUD2)
                        Store (0xAA, RFLG)
                    }
                ...
                If (LAnd (LEqual (SAFN, 0x5843), LEqual (SASB, 0x91)))
                {
                    ...
                    If (LAnd (LEqual (VGTY, 0x02), LEqual (M110, One)))
                    {
                        If (LEqual (CAID, ToUUID ("8246028d-8bca-4a55-ba0f-6f1e6b921b8f")))
                        {
                            If (LEqual (FNCN, 0x50))
                            {
                                Switch (SUBN)
                                {
                                    ...
                                    Case (One)
                                    {
                                        ...
                                        If (LEqual (VGTY, 0x02))
                                        {
                                            Notify (^^PCI0.RP01.PEGP, 0xD1)
                                            Store (One, NVGF)
                                            Notify (^^PCI0.RP01.PEGP, 0xC0)
                                        }
                    ...
                    If (LEqual (CAID, ToUUID ("415784c7-8236-45ac-ba08-8d5dfa86d094")))
                    {
                        If (LEqual (FNCN, 0x50))
                        {
                            Switch (SUBN)
