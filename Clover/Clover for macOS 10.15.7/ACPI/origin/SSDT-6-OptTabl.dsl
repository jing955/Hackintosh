 ...
 *
 * Disassembly of SSDT-6-OptTabl.aml, Thu Dec 17 23:43:36 2020
 *
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    External (_SB_.PCI0.LPCB.H_EC.CTMP, UnknownObj)    // (from opcode)
    ...
    External (_SB_.PCI0.SMUT, MutexObj)    // (from opcode)
    External (BEST, UnknownObj)    // (from opcode)
    ...
    External (NVTP, FieldUnitObj)    // (from opcode)
    ...
    External (VGTY, FieldUnitObj)    // (from opcode)
                    ...
                    Switch (PSH0)
                    {
                        Case (Zero)
                        {
                            If (CPUT)
                            {
                                ...
                                If (LEqual (ECON, One))
                                {
                                    Store (\_SB.PCI0.LPCB.H_EC.CTMP, PDTS)
                                }
                            }
